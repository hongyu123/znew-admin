<template>
  <div class="upload-box">
    ...
    >
      ...
      <template #file="{ file }">
        <div @mouseenter.stop="imgDrop(file.uid)">
          <img :src="file.url" class="upload-image" />
          <div
            :class="['upload-handle', _fileList.length > 1 && '!cursor-move']"
            @click.stop
          >
...
const beforeUpload: UploadProps["beforeUpload"] = rawFile => {
  ...
  const imgType = rawFile.type.startsWith("image");
  if (!imgType)
    ElNotification({
      title: "温馨提示",
...
const imgDrop = uid => {
  ...
  const _curIndex = _fileList.value.findIndex(img => img.uid === uid);
  const wrapper: HTMLElement = document.querySelector(`.${CLASSNAME}`);
  Sortable.create(wrapper, {
    handle: `.${CLASSNAME}__item`,
    onEnd: ({ newIndex, oldIndex }) => {
      const oldFile = _fileList.value[oldIndex];
      _fileList.value.splice(oldIndex, 1);
      _fileList.value.splice(newIndex, 0, oldFile);
    }
  });
};
</script>
