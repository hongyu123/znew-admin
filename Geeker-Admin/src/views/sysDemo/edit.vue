<template>
  <el-drawer v-model="drawerVisible" :destroy-on-close="true" size="50%" :title="`${drawerProps.title}系统示例表`">
    ...
    >
      <el-form-item label="名称xx" prop="name">
...
const rules = reactive({
  ...
  registTime: [{ required: true, message: "请填写注册时间", trigger: "change" }],
  avatar: [{ required: true, message: "请填写头像", trigger: "change" }],
  introduction: [{ required: true, message: "请填写简介", trigger: "change" }],
  phone: [{ required: false, pattern: /^\d{11}$/, message: "手机格式错误", trigger: "change" }],
  lat: [{ required: true, message: "请填写纬度", trigger: "change" }],
  lng: [{ required: true, message: "请填写经度", trigger: "change" }]
});
