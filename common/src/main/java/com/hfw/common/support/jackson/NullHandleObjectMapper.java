 ...
 * null处理的ObjectMapper
 */
public class NullHandleObjectMapper{
    private NullHandleObjectMapper(){}
    private volatile static ObjectMapper objectMapper;
    public static ObjectMapper getInstance(){
        if(objectMapper==null){
            synchronized (NullHandleObjectMapper.class){
                if(objectMapper==null){
                    objectMapper = build();
                    //不使用@JsonFilter
                    FilterProvider filterProvider = new SimpleFilterProvider().addFilter(ApiResult.includeFilter, SimpleBeanPropertyFilter.serializeAll());
